    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-6">
        <div className="relative">
          ...
          <button
            className="absolute bottom-0 right-0 p-2 bg-white border border-neutral-200 rounded-full shadow-sm hover:bg-neutral-50 transition-colors"
            title="Cambiar foto"
          >
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {profile.roles.slice(0, 3).map((role) => (
              <span
                key={role}
                className="px-2 py-0.5 text-xs bg-gold-light text-gold-dark rounded-full"
              >
                {role}
              </span>
            ))}
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-neutral-200">
        ...
        {saveSuccess && (
          <span className="text-sm text-success-dark">Cambios guardados correctamente</span>
        )}
      </div>
    </div>
  )
